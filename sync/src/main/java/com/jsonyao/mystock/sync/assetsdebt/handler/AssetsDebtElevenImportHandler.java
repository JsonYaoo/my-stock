package com.jsonyao.mystock.sync.assetsdebt.handler;
...
/**
 * 资产负债表 理杏仁11字段 Excel处理程序
 *
 * @author yaocs2
 * @since 20220502
 */
@Slf4j
public class AssetsDebtElevenImportHandler implements ElevenImportHandler<SyncAssetsDebtDTO> {

    private static final String SPLITTER = "#$#";

    /** 季度+股票代码缓存 */
    private static ThreadLocal<Map<String, SyncAssetsDebtDTO>> QUA_CODE_CACHE = ThreadLocal.withInitial(HashMap::new);

    @Override
    public List<SyncAssetsDebtDTO> handle(List<LixingrenExcelUtil.ElevenExcel> elevenExcelList) {
        log.info("处理所有资产负债表所有的excel~");
        try {
            ...
            // 返回更新好的缓存数
            Map<String, SyncAssetsDebtDTO> cacheMap = QUA_CODE_CACHE.get();
    ...
    private static void doHandle(Integer year, Integer quarter, String filedDesc, List<ElevenImportBean> beanList) {
        ...
        // 更新缓存数据
        for (int i = 0; i < 11; i++) {
            log.info("处理资产负债表的Bean: year={}, quarter={}, filedDesc={}", year, quarter, filedDesc);
            ...
            for (ElevenImportBean bean : beanList) {
                ...
                if(!QUA_CODE_CACHE.get().containsKey(key)) {
                    SyncAssetsDebtDTO initDto = new SyncAssetsDebtDTO();
                ...
                SyncAssetsDebtDTO dto = QUA_CODE_CACHE.get().get(key);
